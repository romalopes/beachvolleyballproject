...
import { createCustomFocus, createSkillFocus, moveFocus, type FocusDraft } from './focusDraft';
...
export default function SkillFocusSelector({
  ...
  const handleAdd = () => {
    ...
    onChange([...focuses, createCustomFocus(customText.trim(), description.trim())]);
    setCustomText('');
    setDescription('');
  };

  return (
    <div className="training-editor-section" role="group" aria-label="Training Focuses">
      <h3>Training Focuses</h3>
      {focuses.length === 0 ? (
        <p className="related-item-meta">No focuses yet. Add an existing skill or a custom focus.</p>
      ) : (
        <ol className="training-focus-list">
          {focuses.map((focus, index) => (
            <li key={focus.key} className="training-focus-item">
              <span className="training-focus-title">
                {index + 1}. {focus.skill_id ? skillName(focus.skill_id) : focus.custom_focus}
              </span>
              {!focus.skill_id && <span className="related-item-meta"> · Custom focus</span>}
              ...
              <div className="training-editor-row-actions">
                ...
                <button
                  ...
                  onClick={() => onChange(focuses.filter((item) => item.key !== focus.key))}
                >
                  <Trash2 size={14} />
                </button>
              </div>
      ...
      <div className="training-focus-form">
        <h4>Add Focus</h4>
        <div className="training-focus-type" role="radiogroup" aria-label="Focus type">
          <label>
            <input
              type="radio"
              name="focus-type"
        ...
        <button type="button" className="admin-btn admin-btn-add" onClick={handleAdd}>
          Add focus
        </button>
      </div>
    </div>
